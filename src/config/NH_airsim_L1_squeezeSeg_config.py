...
def NH_airsim_L1_squeezeSeg_config():
  """Specify the parameters to tune below."""
  mc                    = base_model_config('NH_airsim')

  # mc.CLASSES            = ['ignore','building','car','road']
  mc.CLASSES            = ['ignore','building','car']
  print("xxxx CLASSES to train on ", mc.CLASSES," xxxxxx")
  mc.NUM_CLASS          = len(mc.CLASSES)
  mc.CLS_2_ID           = dict(zip(mc.CLASSES, range(len(mc.CLASSES))))
  # mc.CLS_LOSS_WEIGHT    = np.array([0.4, 2.0, 2.0,  1.0 ])
  mc.CLS_LOSS_WEIGHT    = np.array([0.4, 2.0, 2.0])
...
# ??????
  ...
  # mc.BILATERAL_THETA_R  = np.array([.015, .01, .01, .015]) 
